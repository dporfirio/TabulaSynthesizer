...
class Planner:
	...
	def get_current_neighbors(self, current, act_seq, hint_list, detached_entities):
	...
	def astar(self, start, act_seq, hint_list, detached_entities, solutions):
		...
		while len(open_set) > 0:
			...
			if len(current[0]) > max(10, len(act_seq) * 3):
				...
				print(current[2])
				print("~~~~~~~~~~~~~~~")
				neighbors = self.get_current_neighbors(current, act_seq, hint_list, detached_entities)
				print()
				#exit()
			#time.sleep(1)
			print("neighbors:")
