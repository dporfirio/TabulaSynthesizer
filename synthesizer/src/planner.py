...
class Planner:
			...
			for action_type in act_data["action_types"]:
				...
				for arg_combo in arg_combos:
					...
					if not any([op.is_match(act) for op in operators]):
						#if act.name == "say" and act.args["speech"].label == SpeechEntity:
						#	print("what do I do")
						#	exit()
						continue
					#else:
	...
	def heuristic(self, curr, act_seq, hint_list, detached_entities):
		...
		#	can_be_sat = True
		#	preconds = hint.precondition._or[0]
		#	for precond_name, precond_val in preconds.items():
		#		sat = False
	...
	def astar(self, start, act_seq, hint_list, detached_entities, solutions, operators):
		...
		while len(open_set) > 0:
			...
			act_hist = current[0]
			sat_spec = True
			#for i, act in enumerate(act_hist):
			#	if i == 0 and act.name != "idle":
			#		sat_spec = False
			#		break
			#	if (i == 1 and act.name != "moveTo") or (i == 1 and act.name == "moveTo" and act.args["destination"].label.name != "garage"):
			#		sat_spec = False
			#		break
			#	if (i == 2 and act.name != "say"):
			#		sat_spec = False
			#		break
	...
	def get_allowable_operators(self, hint_list, detached_entities, act_seq):
		...
		print([str(op) for op in operators])
		return operators
